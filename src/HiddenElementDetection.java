...
public class HiddenElementDetection {
    public static void main(String[] args) throws InterruptedException {
        ...
        int counter = 0; // Counter to track websites with hidden elements
        ...
        for (String url : websitesToTest) {
            try {
                ...
                List<WebElement> initialVisibleElements = driver.findElements(By.cssSelector("*"));
                initialVisibleElements.removeIf(element -> !element.isDisplayed());

                System.out.println("Total number of initially visible elements: " + initialVisibleElements.size());

                Thread.sleep(5000); // 10 seconds delay

                int hiddenCount = 0;
                for (WebElement element : initialVisibleElements) {
                    if (!element.isDisplayed()) {
                        hiddenCount++;
                    }
                }

                System.out.println("Total number of elements that became hidden: " + hiddenCount);
        ...
        System.out.println("Number of websites with fewer hidden elements after: " + counter);
    }
}
